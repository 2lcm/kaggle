...
class ASLModel_(nn.Module):
    def __init__(self, x_dim, y_dim, d_model, nhead, d_hid, nlayers, dropout):
        super().__init__()
    ...
    def inference(self, src, y_len):
        tgt = torch.ones(src.size(0), 1, dtype=torch.long, device=src.device)
        ...
        return out
ASLModel = ASLModel_(
    x_dim=84, y_dim=62, d_model=128, nhead=8, d_hid=512, nlayers=6, dropout=0.1
)
...
if __name__ == "__main__":
    ...
    bs = 16
    y_len = 64
    x = torch.randn(bs, 512, 84, device='cuda')
    y = torch.randint(low=0, high=62, size=(bs, y_len), device='cuda')
