...
class ASLModel_(nn.Module):
    def __init__(self, x_dim, y_len, y_dim, d_model, n_head, n_enc_layers, n_dec_layers, d_ff, dropout):
        ...
        self.fc_cls = nn.Linear(d_model, y_len)
    ...
    def forward(self, kps):
        ...
        de_mask = self.generate_mask(kps.size(1))
        de_out = self.decoder(kps, en_out, de_mask) # b, x_l, d_model
        out_1 = F.softmax(self.fc_cls(de_out), dim=-1) # b, x_l, y_l
        out_2 = F.softmax(self.fc_dim(de_out), dim=-1) # b, x_l, y_dim
        out = torch.matmul(out_1.permute(0, 2, 1), out_2)
        return out
    ...
    def inference(self, kps):
        out = self.forward(kps)
        out = torch.argmax(out, dim=-1)
        return out

ASLModel = ASLModel_(
    x_dim=150, y_dim=60, y_len=64, d_model=512, n_head=8, 
