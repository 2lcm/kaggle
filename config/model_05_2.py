...
class ASLModel_(nn.Module):
    ...
    def inference(self, src):
        tgt = torch.zeros(src.size(0), self.y_len, dtype=torch.long, device=src.device)
        tgt[:, 0] = 1 # <sos>
...
ASLModel = ASLModel_(
    x_dim=84, y_dim=62, y_len=64, d_model=128, nhead=1, d_hid=512, nlayers=12, dropout=0.1
)
